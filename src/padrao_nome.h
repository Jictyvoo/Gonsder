...
int verifica_padrao(char nome_arquivo[], char padrao_nome[]) {
    ...
    int quantidade_TOTAL = (int) strlen(padrao_nome);
    int quantidade_compara = (int) strlen(nome_arquivo);
...
unsigned short int nomainus(char nome_arquivo[], char padrao_nome[], char termino) {
    ...
    do {
        ...
        if ((pra_comparar[i] >= 48 && pra_comparar[i] <= 57) && (padrao_nome[i] >= 48 && padrao_nome[i] <= 57))
            auxiliar = 0;
        else if ((pra_comparar[i] == padrao_nome[i]) || (pra_comparar[i] == (padrao_nome[i] + 32)) ||
                 (pra_comparar[i] == (padrao_nome[i] - 32)))
            auxiliar = 0;
        else
            return 1;
        i = i + 1;
