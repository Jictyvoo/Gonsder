...
enum escolhas{atacar, skill_attack, conversar, mochila, chorar};
enum sucesso{vitoria, derrota, seduzido};
enum leveis_sobe{upa_nivel, same_nivel};
...
void level_up(){
	...
	SKILL_CHARACTER.TOTAL_MANA = (long int) SKILL_CHARACTER.TOTAL_MANA + (rand() % (15 + (MAIN_CHARACTER.EXPERIENCIA/(100 * (MAIN_CHARACTER.NIVEL + 1))) ) );
...
void apresenta_companheiros(status_friends *COMPANHEIROS){
	...
	while(temporaria < 4){
		if(COMPANHEIROS -> existentes[temporaria] != derrota){
			fprintf(stdout, "%c)\n", 65 + temporaria);
			fprintf(stdout, "VIDA: %ld/%ld\n", COMPANHEIROS -> THEY_HAVE[temporaria].have_life, COMPANHEIROS -> THEY_TOTAL[temporaria].VIDA);
			fprintf(stdout, "AGILIDADE: %ld/%ld\n", COMPANHEIROS -> THEY_HAVE[temporaria].have_agility, COMPANHEIROS -> THEY_TOTAL[temporaria].AGILIDADE);
			fprintf(stdout, "DEFESA: %ld/%ld\n", COMPANHEIROS -> THEY_HAVE[temporaria].have_defense, COMPANHEIROS -> THEY_TOTAL[temporaria].DEFESA);
			fprintf(stdout, "ATAQUE: %ld/%ld\n", COMPANHEIROS -> THEY_HAVE[temporaria].have_attack, COMPANHEIROS -> THEY_TOTAL[temporaria].ATAQUE);
			fprintf(stdout, "CHARME: %ld/%ld\n", COMPANHEIROS -> THEY_HAVE[temporaria].have_luck, COMPANHEIROS -> THEY_TOTAL[temporaria].CHARME);
			fprintf(stdout, "CARISMA: %ld/%ld\n", COMPANHEIROS -> THEY_HAVE[temporaria].have_charism, COMPANHEIROS -> THEY_TOTAL[temporaria].CARISMA);
...
void selecao_mochila(atributos *PRINCIPAL, mochilinha *O_PERSONAGEM, tem_status *PERTEM, the_skills *SKILL_PERSONAGEM, char vilao_frases[][100], short int numero){
...
int conversa_diminui(atributos *PRINCIPAL, tem_status *PERTEM, tem_status *HAVE_PERSONAGEM, atributos *MAIN_PERSONAGEM, status_friends *COMPANHEIROS, char vilao_frases[][100], unsigned short int numero){
...
unsigned short int character_skills(tem_status *HAVE_PERSONAGEM, atributos PERSONAGEM, the_skills *SKILL_PERSONAGEM, unsigned short int *contador){
	unsigned short int escolha;
	do{
		tecla_buff();
		limpa_tela();
...
unsigned short int controle_skill_duracao(unsigned short int contador[], tem_status *HAVE_PERSONAGEM, atributos PERSONAGEM){
...
unsigned short int combat_system(atributos VILAO, tem_status VILOA, char vilao_frases[][100], unsigned short int chance, short int numero, status_friends *COMPANHEIROS){
	unsigned short int escolha;
	unsigned short int controle = 0;
	...
	do{
		...
		switch(escolha){
			case atacar:
				...
				fprintf(stdout, "%s\n\n", vilao_frases[rand() % numero]);
				printf("%s LIFE: %ld/%ld\n%s LIFE: %ld/%ld\n", MAIN_CHARACTER.NOME_PERSONAGEM, HAVE_CHARACTER.have_life, MAIN_CHARACTER.VIDA, VILAO.NOME_PERSONAGEM, VILOA.have_life, VILAO.VIDA);
			...
			case conversar:
				if(conversa_diminui(&VILAO, &VILOA, &HAVE_CHARACTER, &MAIN_CHARACTER, COMPANHEIROS, vilao_frases, numero) == vitoria)
					VILOA.have_life = 0;
				
				break;

			case mochila:
				selecao_mochila(&MAIN_CHARACTER, &MAIN_PERSONAGEM, &HAVE_CHARACTER, &SKILL_CHARACTER, vilao_frases, numero);
				break;
...
unsigned short int menu_aventura(char historia_frases[], unsigned int trecho_historia, status_friends *COMPANHEIROS){
	...
	do{
		...
		limpa_tela();
		fprintf(stdout, "%s\n", historia_frases);
		fprintf(stdout, "[1] CONTINUAR HISTORIA\n[2] TREINAR CONTRA MONSTROS\n[3] STATUS PERSONAGEM\n[4] DESCANSAR\n[5] SALVAR E SAIR\n--> ");
...
int main(){
	...
	while(controle != 3){
		...
		if(controle == 1){
			...
			limpa_tela();
			fprintf(stdout, "Digite os numeros correspondentes as acoes que deseja tomar\n escolha suas acoes e acate a zoeira\n");
			continuar_enter();
		}
	}
	return chorar;
